 ...
 * three rules are enforced by every module's own signup schema, which means
 * the meter never shows "Fair" for a password the server will then reject.
 *
 * Shared (not owned by auth or any one module) because every self-service
 * sign-up path — staff, rider, vendor — reuses the same PasswordStrengthMeter.
 */
